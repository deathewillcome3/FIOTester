...
def populate_args(arg, parent_file, head, tail):
    return arg + str(parent_file.get(arg[head:tail])) if arg[head:tail] in parent_file \
    else arg
...
def run_test_from_file(os, device, name, yaml_conf, defaults):
    ...
    for coordinate in product(eqdepths, params.get("blocksizes"), params.get("threads"), params.get("qdepths"),
        ...
        if coordinate[2]*coordinate[3] == coordinate[0]:
            ...
            arguments[len(arguments) - 1] += str(coordinate[1]) + "-" + str(coordinate[2]) + "-" + str(
                coordinate[3]) + "-" + str(coordinate[4])
            arguments[len(arguments) - 2] += str(coordinate[1]) + "-" + str(coordinate[2]) + "-" + str(
                coordinate[3]) + "-" + str(coordinate[4])
            print("Running this command" + str(arguments))
            if os == "Windows":
                output = subprocess.Popen(["powershell.exe", "fio", "--output-format=json", "--thread"] + arguments, cstdout=subprocess.PIPE, stderr=subprocess.PIPE)
            else:
                output = subprocess.Popen(["fio", "--output-format=json"] + arguments, stdout=subprocess.PIPE, stderr=subprocess.PIPE)